...
def convert_sentence_to_token(sentence, seq_length, tokenizer):
    ...
    start_pos =  len(tokenized_text)  + (2 if (USE_SEP or USE_SO) else 1)
...
def convert_whole_word_to_feature(tokens_a, mask_position, seq_length, tokenizer, prob_mask):
    ...
    for token in tokens_a:
        tokens.append(token)
        input_type_ids.append(1 if USE_SEP else 0)
...
def getWordCount(word_count_path):
    word2count = {}
    if word_count_path.endswith('.xlsx'):
        ...
        assert word_count_path.endswith('.tsv')
...
def get_score(sentence,tokenizer,maskedLM):
    ...
    for i,word in enumerate(tokenize_input):
        ...
        #print(mask_input)
#CPU        mask_input = mask_input.to('cuda')
        with torch.no_grad():
            if USE_HUGGINGFACE:
                pre_word =maskedLM(input_ids=mask_input)['logits']    # ADNO, attention unused
            else:
                __, pre_word =maskedLM(mask_input)
...
#             attention_mask = attention_mask.to('cuda')
            ...
            with torch.no_grad():
                ...
                if USE_HUGGINGFACE:
                    ...
                    prediction_scores   = out_dict['logits']
                else:
